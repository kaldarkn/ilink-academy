import styles from './Reviews.module.scss';
import ReviewCard from '../ReviewCard';
import React, { useEffect } from 'react';
import { useState } from 'react';
import { useRef } from 'react';

//Данные с авторами и комментариями (допустим получили с бэка)
const comments = [
  {
    name: 'Буба Бубенцов',
    photo: require('../../assets/img/author1.png'),
    date: '08.01.2022',
    comment:
      'Отличный коллектив, руководители понимают сам процесс работы каждого сотрудника и помогают всем без исключения. Система KPI позволяет реально хорошо зарабатывать по простому принципу - чем больше и лучше ты работаешь, тем больше денег получаешь. Соцпакет - отличная страховка ДМС, организовали курсы английского языка бесплатно, оплачивают тренажерный зал. Зарплату выплачивают всегда вовремя.',
  },
  {
    name: 'Илья Анташкевич',
    photo: require('../../assets/img/noPhoto.png'),
    date: '08.01.2022',
    comment:
      'Год назад попытал счастье, откликнулся на вакансию, прошел собес и попал в компанию. Долго переживал что будет тяжело влиться, но тут прям классные ребята работают, все на одной волне. Всегда готовы помочь с любым вопросом. Для эффективной работы здесь нужно хорошо знать иностранные языки.',
  },
  {
    name: 'Юрина Маргарита',
    photo: require('../../assets/img/author3.png'),
    date: '26.12.2021',
    comment:
      'Наша компания благодарна фирме ilink за сотрудничество. Хотелось бы отметить отличную работу сотрудников: все было выполнено качественно, со знанием дела, в установленные сроки. За время работы с компанией значительно увеличилась аудитория, сайт приносит стабильный доход, контент уникальный, грамотный и качественный. Будет продолжать работу и дальше. Мы довольны, что доверили создание сайта компании ilink.',
  },
  {
    name: 'Дмитрий Иванов',
    photo: require('../../assets/img/author4.png'),
    date: '16.12.2021',
    comment:
      'Отвечала за найм и адаптацию сотрудников в компании, за поддержание на нужном уровне HR-бренда и трудового настроя коллектива. В компанию пришла без опыта работы HR-ом. Всему научилась здесь. Как теории, так и практике. Набрала опыт достаточно быстро. Есть программа обучения, поощряются различные курсы, и это большой плюс. В коллективе очень дружная атмосфера. Все дружелюбные, амбициозные.',
  },
];

const Reviews = ({ handlerOpenForm }) => {
  let [numberOfSliderPages, setNumberOfSliderPages] = useState(comments.length / 2); //кол-во слайдов == кол-во фидбэков/2 (если с ПК)
  let [currentSliderPage, setCurrentSliderPage] = useState(0); //текущая активная страница слайдера
  let sliderWindowWidth = useRef(null); //ширина окна слайдера

  useEffect(() => {
    //При первом рендере получаю размер окна слайдера
    sliderWindowWidth.current = document.getElementById('window').offsetWidth;

    //Если мобильная версия браузера (width < 540px), то количество слайдов равна количеству фидбэков
    if (window.screen.width <= 540) {
      setNumberOfSliderPages(comments.length);
    } else {
      //Иначе количество слайдов равна половине количества фидбэков
      setNumberOfSliderPages(comments.length / 2);
    }
  }, []);

  window.addEventListener('resize', () => {
    //При изменении разрешения, заново получаю размер окна слайдера
    sliderWindowWidth.current = document.getElementById('slider').offsetWidth;

    //Если мобильная версия браузера (width < 540px), то количество слайдов равна количеству фидбэков
    if (window.screen.width <= 540) {
      setNumberOfSliderPages(comments.length);
    } else {
      //Иначе количество слайдов равна половине количества фидбэков
      setNumberOfSliderPages(comments.length / 2);
    }
    //Устанавливаем активным слайдом самый первый
    setCurrentSliderPage(0);
  });

  const clickNext = () => {
    if (currentSliderPage < comments.length / 2 - 1) {
      setCurrentSliderPage(currentSliderPage + 1);
    } else {
      setCurrentSliderPage(0);
    }
  };

  const clickPrev = () => {
    if (currentSliderPage > 0) {
      setCurrentSliderPage(currentSliderPage - 1);
    } else {
      setCurrentSliderPage(comments.length / 2 - 1);
    }
  };

  return (
    <div className={styles.wrapper}>
      <div className={styles.reviews}>
        <div className={styles.header}>
          <h1>Отзывы</h1>
          <svg
            onClick={() => handlerOpenForm()}
            className={styles.bigBtn}
            viewBox="0 0 220 52"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <rect width="220" height="52" rx="2" fill="#585CC6" />
            <path
              d="M36.1286 25.1286V19H38.1714V25.1286H44.3V27.1714H38.1714V33.3H36.1286V27.1714H30V25.1286H36.1286Z"
              fill="white"
            />
            <path
              d="M66.71 18.4V29.02H68.06V33.412H65.99V31H58.34V33.412H56.27V29.02H57.44C58.172 28.072 58.538 26.626 58.538 24.682V18.4H66.71ZM59.708 29.02H64.64V20.344H60.608V24.718C60.608 26.59 60.308 28.024 59.708 29.02ZM77.18 29.866C76.268 30.778 75.146 31.234 73.814 31.234C72.482 31.234 71.36 30.778 70.448 29.866C69.524 28.942 69.062 27.82 69.062 26.5C69.062 25.18 69.524 24.058 70.448 23.134C71.36 22.222 72.482 21.766 73.814 21.766C75.146 21.766 76.268 22.222 77.18 23.134C78.104 24.058 78.566 25.18 78.566 26.5C78.566 27.82 78.104 28.942 77.18 29.866ZM71.006 26.5C71.006 27.316 71.276 27.994 71.816 28.534C72.356 29.074 73.022 29.344 73.814 29.344C74.606 29.344 75.272 29.074 75.812 28.534C76.352 27.994 76.622 27.316 76.622 26.5C76.622 25.684 76.352 25.006 75.812 24.466C75.272 23.926 74.606 23.656 73.814 23.656C73.022 23.656 72.356 23.926 71.816 24.466C71.276 25.006 71.006 25.684 71.006 26.5ZM84.6798 31.234C83.3118 31.234 82.1838 30.79 81.2958 29.902C80.4318 29.002 79.9998 27.856 79.9998 26.464C79.9998 24.952 80.2278 23.776 80.6838 22.936C81.1278 22.108 82.0518 21.346 83.4558 20.65C83.5038 20.626 83.5818 20.59 83.6898 20.542C83.8098 20.482 83.9358 20.422 84.0678 20.362C84.1998 20.302 84.3318 20.242 84.4638 20.182C84.9678 19.954 85.2798 19.81 85.3998 19.75C85.4718 19.714 85.6818 19.582 86.0298 19.354C86.4498 19.054 86.7318 18.628 86.8758 18.076L88.4598 18.994C88.3998 19.654 87.9858 20.26 87.2178 20.812C87.0858 20.932 86.8398 21.076 86.4798 21.244L85.7058 21.604L84.8058 21.982C84.0258 22.294 83.4498 22.612 83.0778 22.936C82.7058 23.26 82.4478 23.674 82.3038 24.178C83.0238 23.362 84.0018 22.954 85.2378 22.954C86.2938 22.954 87.2118 23.332 87.9918 24.088C88.7838 24.844 89.1798 25.822 89.1798 27.022C89.1798 28.246 88.7598 29.254 87.9198 30.046C87.0918 30.838 86.0118 31.234 84.6798 31.234ZM84.6618 29.416C85.4178 29.416 86.0358 29.194 86.5158 28.75C86.9958 28.294 87.2358 27.718 87.2358 27.022C87.2358 26.314 86.9958 25.732 86.5158 25.276C86.0478 24.82 85.4358 24.592 84.6798 24.592C84.0078 24.592 83.4018 24.808 82.8618 25.24C82.3218 25.648 82.0518 26.212 82.0518 26.932C82.0518 27.652 82.3038 28.246 82.8078 28.714C83.3118 29.182 83.9298 29.416 84.6618 29.416ZM98.0247 23.278V22H99.9687V31H98.0247V29.704C97.2927 30.724 96.2427 31.234 94.8747 31.234C93.6387 31.234 92.5827 30.778 91.7067 29.866C90.8307 28.954 90.3927 27.832 90.3927 26.5C90.3927 25.168 90.8307 24.046 91.7067 23.134C92.5827 22.222 93.6387 21.766 94.8747 21.766C96.2427 21.766 97.2927 22.27 98.0247 23.278ZM93.1467 28.57C93.6867 29.11 94.3647 29.38 95.1807 29.38C95.9967 29.38 96.6747 29.11 97.2147 28.57C97.7547 28.018 98.0247 27.328 98.0247 26.5C98.0247 25.672 97.7547 24.988 97.2147 24.448C96.6747 23.896 95.9967 23.62 95.1807 23.62C94.3647 23.62 93.6867 23.896 93.1467 24.448C92.6067 24.988 92.3367 25.672 92.3367 26.5C92.3367 27.316 92.6067 28.006 93.1467 28.57ZM108.643 26.374C109.531 26.794 109.975 27.484 109.975 28.444C109.975 29.212 109.711 29.83 109.183 30.298C108.667 30.766 107.947 31 107.023 31H102.415V22H106.663C107.575 22 108.289 22.228 108.805 22.684C109.321 23.14 109.579 23.74 109.579 24.484C109.579 25.336 109.267 25.966 108.643 26.374ZM106.519 23.656H104.269V25.654H106.519C107.311 25.654 107.707 25.318 107.707 24.646C107.707 23.986 107.311 23.656 106.519 23.656ZM104.269 29.308H106.843C107.659 29.308 108.067 28.954 108.067 28.246C108.067 27.934 107.959 27.694 107.743 27.526C107.527 27.346 107.227 27.256 106.843 27.256H104.269V29.308ZM113.643 27.706L118.251 22H119.745V31H117.837V25.276L113.229 31H111.735V22H113.643V27.706ZM121.227 22H129.057V23.872H126.123V31H124.179V23.872H121.227V22ZM132.496 25.06H134.476C135.448 25.06 136.234 25.336 136.834 25.888C137.446 26.428 137.752 27.142 137.752 28.03C137.752 28.906 137.446 29.62 136.834 30.172C136.234 30.724 135.448 31 134.476 31H130.552V22H132.496V25.06ZM132.496 29.254H134.494C134.878 29.254 135.196 29.14 135.448 28.912C135.712 28.684 135.844 28.39 135.844 28.03C135.844 27.67 135.718 27.376 135.466 27.148C135.214 26.92 134.89 26.806 134.494 26.806H132.496V29.254ZM151.815 29.866C150.903 30.778 149.781 31.234 148.449 31.234C147.117 31.234 145.995 30.778 145.083 29.866C144.159 28.942 143.697 27.82 143.697 26.5C143.697 25.18 144.159 24.058 145.083 23.134C145.995 22.222 147.117 21.766 148.449 21.766C149.781 21.766 150.903 22.222 151.815 23.134C152.739 24.058 153.201 25.18 153.201 26.5C153.201 27.82 152.739 28.942 151.815 29.866ZM145.641 26.5C145.641 27.316 145.911 27.994 146.451 28.534C146.991 29.074 147.657 29.344 148.449 29.344C149.241 29.344 149.907 29.074 150.447 28.534C150.987 27.994 151.257 27.316 151.257 26.5C151.257 25.684 150.987 25.006 150.447 24.466C149.907 23.926 149.241 23.656 148.449 23.656C147.657 23.656 146.991 23.926 146.451 24.466C145.911 25.006 145.641 25.684 145.641 26.5ZM153.834 22H161.664V23.872H158.73V31H156.786V23.872H153.834V22ZM167.953 26.392C168.913 26.824 169.393 27.538 169.393 28.534C169.393 29.314 169.075 29.962 168.439 30.478C167.803 30.982 166.939 31.234 165.847 31.234C164.035 31.234 162.811 30.514 162.175 29.074L163.759 28.138C164.083 29.038 164.785 29.488 165.865 29.488C166.369 29.488 166.765 29.386 167.053 29.182C167.341 28.966 167.485 28.684 167.485 28.336C167.485 28.024 167.371 27.778 167.143 27.598C166.915 27.406 166.597 27.31 166.189 27.31H164.911V25.6H165.865C166.261 25.6 166.567 25.51 166.783 25.33C167.011 25.138 167.125 24.88 167.125 24.556C167.125 24.244 166.999 23.992 166.747 23.8C166.495 23.596 166.159 23.494 165.739 23.494C164.839 23.494 164.215 23.89 163.867 24.682L162.283 23.782C162.943 22.438 164.083 21.766 165.703 21.766C166.711 21.766 167.509 22.024 168.097 22.54C168.697 23.044 168.997 23.662 168.997 24.394C168.997 25.294 168.649 25.96 167.953 26.392ZM173.105 25.06H175.085C176.057 25.06 176.843 25.336 177.443 25.888C178.055 26.428 178.361 27.142 178.361 28.03C178.361 28.906 178.055 29.62 177.443 30.172C176.843 30.724 176.057 31 175.085 31H171.161V22H173.105V25.06ZM179.441 31V22H181.385V31H179.441ZM173.105 29.254H175.103C175.487 29.254 175.805 29.14 176.057 28.912C176.321 28.684 176.453 28.39 176.453 28.03C176.453 27.67 176.327 27.376 176.075 27.148C175.823 26.92 175.499 26.806 175.103 26.806H173.105V29.254ZM190.05 26.374C190.938 26.794 191.382 27.484 191.382 28.444C191.382 29.212 191.118 29.83 190.59 30.298C190.074 30.766 189.354 31 188.43 31H183.822V22H188.07C188.982 22 189.696 22.228 190.212 22.684C190.728 23.14 190.986 23.74 190.986 24.484C190.986 25.336 190.674 25.966 190.05 26.374ZM187.926 23.656H185.676V25.654H187.926C188.718 25.654 189.114 25.318 189.114 24.646C189.114 23.986 188.718 23.656 187.926 23.656ZM185.676 29.308H188.25C189.066 29.308 189.474 28.954 189.474 28.246C189.474 27.934 189.366 27.694 189.15 27.526C188.934 27.346 188.634 27.256 188.25 27.256H185.676V29.308Z"
              fill="white"
            />
          </svg>
          <svg
            onClick={() => handlerOpenForm()}
            className={styles.smallBtn}
            viewBox="0 0 42 42"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <rect width="42" height="42" rx="2" fill="#585CC6" />
            <path
              d="M20.1286 20.1286V14H22.1714V20.1286H28.3V22.1714H22.1714V28.3H20.1286V22.1714H14V20.1286H20.1286Z"
              fill="white"
            />
          </svg>
        </div>
        <div id="window" className={styles.window}>
          <article
            id="slider"
            className={styles.slider}
            style={{
              transform: `translateX(-${currentSliderPage * sliderWindowWidth.current}px)`,
            }}>
            {comments.map((item, index) => {
              return <ReviewCard key={index} {...item} />;
            })}
          </article>
        </div>

        <div className={styles.dotsContainer}>
          {Array(numberOfSliderPages)
            .fill(0)
            .map((val, index) => {
              return (
                <React.Fragment key={index + 'Fragment'}>
                  <input
                    type="radio"
                    name="radioDots"
                    id={index}
                    onChange={() => setCurrentSliderPage(index)}
                    checked={currentSliderPage === index}
                  />
                  <label htmlFor={index}></label>
                </React.Fragment>
              );
            })}
        </div>
      </div>
      <svg
        onClick={clickPrev}
        className={styles.prevBtn}
        viewBox="0 0 56 56"
        fill="none"
        xmlns="http://www.w3.org/2000/svg">
        <rect width="56" height="56" rx="2" fill="white" />
        {/* #E0E0E0 */}
        <path
          d="M20.25 28.2744L35.25 28.2744"
          // "#8A8A8A"
          stroke="#333333"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M26.2998 34.2988L20.2498 28.2748L26.2998 22.2498"
          stroke="#333333"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
      <svg
        onClick={clickNext}
        className={styles.nextBtn}
        viewBox="0 0 56 56"
        fill="none"
        xmlns="http://www.w3.org/2000/svg">
        <rect width="56" height="56" rx="2" fill="white" />
        <path
          d="M35.75 27.7256H20.75"
          stroke="#333333"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M29.7002 21.7012L35.7502 27.7252L29.7002 33.7502"
          stroke="#333333"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </div>
  );
};

export default Reviews;
